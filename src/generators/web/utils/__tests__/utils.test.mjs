...
test('createChunkedRequire resolves virtual chunks and falls back to require', () => {
  const chunks = [
...
test('createImportDeclaration produces correct import strings', () => {
  // side-effect import
  assert.equal(
  ...
  // default import
  assert.equal(createImportDeclaration('X', './mod'), 'import X from "./mod";');
...
test('builders produce client and server programs containing expected markers', () => {
...
test('css plugin buildEnd is a no-op when no chunks processed', () => {
