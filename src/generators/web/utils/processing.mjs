...
export function convertJSXToCode(
  ...
  for (const entry of entries) {
    ...
    const toJsFn = overrides.toJs || toJs;
    const { value: code } = toJsFn(entry, { handlers: jsx });
...
export async function executeServerCode(
  ...
  const bundler = overrides.bundleCode || bundleCode;
  const { chunks, css } = await bundler(serverCodeMap, { server: true });
  ...
  const createReq = overrides.createChunkedRequire || createChunkedRequire;
  const enhancedRequire = createReq(otherChunks, requireFn);
...
export async function processJSXEntries(
  ...
  const { version, overrides } = options;
  ...
  const [serverBundle, clientBundle] = await Promise.all([
    executeServerCode(serverCodeMap, requireFn, overrides),
    bundler(clientCodeMap),
  ]);
  ...
  const cssTransformer = overrides?.transform || transform;
  const { code: minifiedCSS } = cssTransformer({
...
/**
 * Lower-level helper that accepts pre-built server and client code maps.
 * This allows tests to bypass AST -> code conversion and inject controlled
 * bundles or bundlers via the `overrides` parameter.
 * @param serverCodeMap
 * @param clientCodeMap
 * @param entries
 * @param template
 ...
 * @param overrides
 */
export async function processWithCodeMaps(
  serverCodeMap,
  clientCodeMap,
  entries,
  template,
