...
describe('updateDepth', () => {
  it('increments on opening bracket', () => {
    assert.strictEqual(updateDepth(0, '['), 1);
  });

  it('decrements on closing bracket', () => {
    assert.strictEqual(updateDepth(1, ']'), 0);
  });

  it('leaves depth unchanged for non-bracket characters', () => {
    assert.strictEqual(updateDepth(3, 'x'), 3);
  });
});
